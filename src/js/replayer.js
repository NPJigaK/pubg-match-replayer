...
window.addEventListener('DOMContentLoaded', (event) => {
  const ar = Array.prototype.slice.call(
      document.getElementsByName('switch-tool'));
  for (let e in ar) {
...
async function getFlightPaths(telemetryJson, mapName) {
  ...
  await telemetryJson.filter(function (value, index) {
    if (value.vehicle && value.character && value.common.isGame >= 0.1
        && value.vehicle.vehicleType == "TransportAircraft") {
      if (calcWithinRange((value.character.location.x / 100),
          (mapSize - flightPathMargin), flightPathMargin)
          && calcWithinRange((value.character.location.y / 100),
              (mapSize - flightPathMargin), flightPathMargin)) {
        flightPaths.push({
