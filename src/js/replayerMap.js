...
function updateCircle(blueZone, isGame) {
    ...
    if (blueZoneGraphics) {
        ...
        blueZoneGraphics.geometry.graphicsData[0].shape.radius = blueZone.radius / 100;
                ...
                if (team.visible) {
                    ...
                    for (let i = 0; i < diff; i++) {
                        if (nowTimeMsec > oldTimeMsec) {
                            ...
                            if (interval % 500 == 0) {
                                let diffLocation = _stateAt(interval).playerLocations[member.name];
                                if (interval > member.landingTime) {
                                    member.polygon.points.push(diffLocation.x / 100, diffLocation.y / 100);
                                }
            ...
            _.forEach(team.member, member => {
                if (!member.visible) {
                    ...
                    if (!playerLocation.descended) {
                        member.circleGraphics.geometry.graphicsData[0].shape.x = playerLocation.x / 100;
                        member.circleGraphics.geometry.graphicsData[0].shape.y = playerLocation.y / 100;
                        member.circleGraphics.visible = true;
                        member.circleGraphics.geometry.invalidate();
                        member.basicText.visible = true;
...
function setFlightPath(flightPath) {
    ...
    let endTriangle = new PIXI.Sprite(PIXI.Texture.from(`../src/assets/icons/flightPath-triangle.png`));
...
function InitTeamGraphics(roster, playerLocations) {
    ...
    _.forEach(roster, team => {
        ...
        _.forEach(team, player => {
            ...
            let basicText = new PIXI.Text(playerNum, new PIXI.TextStyle({align: "center", fontSize: 70}));
...
function InitMap(mapName) {
    ...
    viewport
        ...
        .clamp({top: 0, left: 0, right: worldWidth, bottom: worldHeight, underflow: 'center'})
        .pinch()
        .wheel()
        .clampZoom({minWidth: w, minHeight: h, maxWidth: worldWidth, maxHeight: worldHeight})

    // initial fit world
    resize()

    // add pubg map
    const texture = PIXI.Texture.from(`../src/assets/maps/${originalMapName}.jpg`);
    ...
    function resize() {
        ...
        app.renderer.resize(pixi_container.clientWidth, pixi_container.clientHeight);
