...
(function () {
  ...
  function x(b) {
    ...
    for (f = 0; f < h.length; f += 1) {
      c = h[f], d = b[c], void 0 === d
          ? e[c] = "negative" !== c || e.negativeBefore ? "mark" === c && "."
          !== e.thousand ? "." : !1 : "-" : "decimals" === c ? 0 < d && 8 > d
              && (e[c] = d) : "encoder" === c || "decoder" === c || "edit" === c
          || "undo" === c ? "function" === typeof d && (e[c] = d) : "string"
              === typeof d && (e[c] = d);
    }
    s(e, "mark", "thousand");
    s(e, "prefix", "negative");
...
(function (factory) {
  ...
  function offset(elem, orientation) {
    ...
    return orientation ? (rect.top + pageOffset.y - docElem.clientTop)
...
// Interface
  ...
  function testRange(parsed, entry) {
    ...
    parsed.spectrum = new Spectrum(entry, parsed.snap, parsed.dir,
        parsed.singleStep);
  }
  ...
  function testTooltips(parsed, entry) {
    ...
    if (entry === false) {
      ...
      parsed.tooltips.forEach(function (formatter) {
        if (typeof formatter !== 'boolean' && (typeof formatter !== 'object'
  ...
  function closure(target, options, originalOptions) {
    ...
    function addMarking(spread, filterFunc, formatter) {
      ...
      addClass(element, options.ort === 0 ? options.cssClasses.pipsHorizontal
          : options.cssClasses.pipsVertical);

      function getClasses(type, source) {
        var a = source === options.cssClasses.value,
            orientationClasses = a ? valueOrientationClasses
                : markerOrientationClasses,
      ...
      function getTags(offset, source, values) {
        return 'class="' + getClasses(values[1], source) + '" style="'
      ...
      function addSpread(offset, values) {
        ...
        values[1] = (values[1] && filterFunc) ? filterFunc(values[0], values[1])
            : values[1];

        // Add a marker for every point
        out += '<div ' + getTags(offset, options.cssClasses.marker, values)
            + '></div>';

        // Values are only appended for points marked '1' or '2'.
        if (values[1]) {
          out += '<div ' + getTags(offset, options.cssClasses.value, values)
    ...
    function baseSize() {
      ...
      return options.ort === 0 ? (rect.width || scope_Base[alt]) : (rect.height
          || scope_Base[alt]);
    }
    ...
    function moveHandles(upward, proposal, locations, handleNumbers) {
      ...
      if (handleNumbers.length > 1) {
        ...
        handleNumbers.forEach(function (handleNumber, o) {
          ...
          var to = checkHandlePosition(proposals, handleNumber,
              proposals[handleNumber] + proposal, b[o], f[o]);
    ...
    function eventMove(event, data) {
      ...
      if (navigator.appVersion.indexOf("MSIE 9") === -1 && event.buttons === 0
          && data.buttonsProperty !== 0) {
        return eventEnd(event, data);
      }

      // Check if we are moving up or down
      var movement = (options.dir ? -1 : 1) * (event.calcPoint
          - data.startCalcPoint);
    ...
    function eventStart(event, data) {
      ...
      var moveEvent = attachEvent(actions.move, document.documentElement,
          eventMove, {
      ...
      var endEvent = attachEvent(actions.end, document.documentElement,
          eventEnd, {
            handleNumbers: data.handleNumbers
          });

      var outEvent = attachEvent("mouseout", document.documentElement,
          documentLeave, {
            handleNumbers: data.handleNumbers
          });

      document.documentElement.noUiListeners = moveEvent.concat(endEvent,
          outEvent);
    ...
    function checkHandlePosition(reference, handleNumber, to, lookBackward,
        lookForward) {
    ...
    function setHandle(handleNumber, to, lookBackward, lookForward) {
      ...
      to = checkHandlePosition(scope_Locations, handleNumber, to, lookBackward,
          lookForward);
    ...
    function removeEvent(namespacedEvent) {
      ...
      Object.keys(scope_Events).forEach(function (bind) {
        ...
        if ((!event || event === tEvent) && (!namespace || namespace
            === tNamespace)) {
    ...
    function updateOptions(optionsToUpdate, fireSetEvent) {
      ...
      var updateAble = ['margin', 'limit', 'padding', 'range', 'animate',
        'snap', 'step', 'format'];
