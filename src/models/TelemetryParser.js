...
function parseTelemetry(matchData, telemetry, focusedPlayerName) {
  ...
  const incrementPlayerStateVal = (playerName, path, delta) => {
    setNewPlayerState(playerName,
        {[path]: latestPlayerStates[playerName][path] + delta})
  }
  ...
  { // --- Step One: Iterate through all telemetry data and store known points
    ...
    telemetry.forEach((d, i) => {
      ...
      if (_.get(d, 'character.name')) {
        // original
        let descended = true;
        if (d._T == "LogParachuteLanding") {
          descended = false;
        }
        if (d.vehicle && d.vehicle.vehicleType == "TransportAircraft") {
          descended = false;
        }
        ...
        setNewPlayerLocation(name,
            {x: location.x, y: location.y, descended: descended})
      }
        ...
        const updatedItems = currentItems.reduce((prev, item) => {
          if (item.itemId === d.parentItem.itemId) {
            return [
              ...
              {
                ...item,
                attachedItems: item.attachedItems.filter(
                    ai => ai !== d.childItem.itemId),
              },
            ]
          }
      ...
      if (d._T === 'LogPlayerTakeDamage') {
        ...
        setNewPlayerLocation(d.victim.name,
            {x: d.victim.location.x, y: d.victim.location.y, descended: true})
        ...
        if (d.damageTypeCategory === 'Damage_Gun') {
          ...
          (state[tracerStart]
              || (state[tracerStart] = blankIntervalState())).tracers.push({
            key: i,
        ...
        if (d.attacker && d.attacker.name && d.attacker.name
            !== d.victim.name) {
      ...
      if (d._T === 'LogGameStatePeriodic') {
        ...
        // original
        if (d.common.isGame >= isGameCount) {
          allPhase.push(
              {
                safeZone: curState.safezone,
    ...
    for (let i = 0; i < state.length; i++) {
      ...
      Object.keys(s.playerLocations).forEach(playerName => {
        (playerLocations[playerName]
            || (playerLocations[playerName] = [])).push(i)
      })
  ...
  { // Step Four (c): Copy safe/red zone points and players
    ...
    for (let i = 1; i < state.length; i++) {
      if (!state[i].redzone) {
        state[i].redzone = state[i - 1].redzone
      }
      if (!state[i].safezone) {
        state[i].safezone = state[i - 1].safezone
      }
      ...
      // original
      if (!state[i].isGame) {
        state[i].isGame = state[i - 1].isGame
      }
    }
  ...
  { // Step Four (d): Expand tracers across their lifetime
    ...
    for (let i = 0; i < state.length; i++) {
      _.remove(activeTracers, tracer => i >= tracer.endInterval)
    ...
    for (let i = 0; i < state.length; i++) {
      ...
      s.carePackages.forEach(cp => { // eslint-disable-line no-loop-func
        ...
        if (cp.state === 'landed') {
          ...
          const matchingCp = _.minBy(cpDistances, 'distance')
          if (matchingCp) {
            activePackages = _.cloneDeep(activePackages)
