...
function Telemetry(state) {
  const getLocation = (interval, playerName, playerLocation) => {
    ...
    return {
      x: linearInterpolation(left.x, right.x, span,
          interval - playerLocation.left),
      y: linearInterpolation(left.y, right.y, span,
          interval - playerLocation.left),
      descended: left.descended
    }
  }
  ...
  const stateAt = msSinceEpoch => {
    ...
    _.forEach(s.players, (player, playerName) => {
      if (!Object.hasOwnProperty.call(player, 'location')) {
        if (Object.hasOwnProperty.call(s.playerLocations[playerName], 'left')) {
          ...
          s.playerLocations[playerName] = getLocation(interval, playerName,
              curLocation)
        }
    ...
    if (Object.hasOwnProperty.call(s.bluezone, 'left')) {
      if (!s.bluezone.right) {
        ...
        s.bluezone = {
          x: linearInterpolation(left.x, right.x, span,
              interval - s.bluezone.left),
          y: linearInterpolation(left.y, right.y, span,
              interval - s.bluezone.left),
          radius: linearInterpolation(left.radius, right.radius, span,
              interval - s.bluezone.left),
        }
      }
    }
  ...
  const finalRoster = () => {
    ...
    //const rosters = _.sortBy(_.groupBy(characters, 'teamId'), r => _.minBy(r, 'ranking').ranking)
    const rosters = _.sortBy(_.groupBy(characters, 'teamId'),
        r => _.minBy(r, 'teamId').teamId)
    ...
    return rosters.map(r => r.map(c => c.name).sort())
  }
